const App = () => {
	...
	t.toast = (message, duration = 2 * 1000) => {
		...
		setTimeout(() => {
			const toastConTop = t.refs.toastCon.style.top === '' ? 0 : Number(t.refs.toastCon.style.top.slice(0, -2))
			...
			if(x){
				...
				setTimeout(() => {
					let toastConBottom = t.refs.toastCon.getBoundingClientRect().bottom
					let messsConTop = t.refs.messsCon.getBoundingClientRect().top
	...
	t.fun = (() => {
		...
		funs.setupControls = () => {
			...
			t.refs.selectLang.append( ...(t.lang.map( (lang, i) => {
				const node = document.createElement('option')
				node.innerText = lang[1]
				node.value = i
				if(t.langSelected === i) node.setAttribute('selected', true)
				return node
			})) )
			t.refs.selectLang.addEventListener('change', e => {
				const index = +e.target.value
				const opts = t.refs.selectLang.querySelectorAll('option[selected]')
				const length = opts.length
				for (let i = 0; i < length; i++) {
					opts[i].removeAttribute('selected')
				}
				t.refs.selectLang.children[index].setAttribute('selected', true)
		...
		s.createInstance = () => {
			...
			for(var i=0; i<sro.length; i++){
				if(sro[i] in window){
					...
					switch(sro[i]){
						case "SpeechRecognition": s.instance = new SpeechRecognition()
							break
						case "webkitSpeechRecognition": s.instance = new webkitSpeechRecognition()
							break
						case "mozSpeechRecognition": s.instance = new mozSpeechRecognition()
							break
						case "msSpeechRecognition": s.instance = new SpeechRecognition()
							break
						case "oSpeechRecognition": s.instance = new oSpeechRecognition()
							break
						default: s.instance = null
							break
					}
					...
					if(s.instance !== null){
						...
						s.instance.onend = () => {
							...
							if(t.refs.currentPara.listening.innerText !== ''){
								const listenedText = t.refs.currentPara.listened.innerText
								let listeningText = t.refs.currentPara.listening.innerText
								
								if(s.cmds[listeningText.toLowerCase()]){
									s.cmds[listeningText.toLowerCase()]()
								}
								else{
									if(t.refs.currentPara.listening.innerHTML !== '<br>'){
										if(listenedText.length === 0 || listenedText.slice(-1) === '.'){
											listeningText = listeningText.slice(0, 1).toUpperCase() + listeningText.slice(1)
										}
										s.transcriptTextArray.push( listeningText )
										t.refs.currentPara.listened.innerText += ( listenedText.length > 0 ? ' ': '' ) + listeningText
						...
						s.instance.onresult = e => {
							...
							if(transcript !== null && transcript !== ''){
								t.refs.currentPara.listening.innerText = transcript
							}
						}	
					...
					(() => {
						s.cmds['cut cut'] = () => {
							if(s.transcriptTextArray.length > 0){
								...
								const newText = (t.refs.currentPara.listened.innerText.slice(0, -(a.length) ) ).trim()
								t.refs.currentPara.listened.innerText = newText
								t.refs.currentPara.listening.innerHTML = newText.length === 0 ? '<br>' : null
							}
							else{
						...
						s.cmds['next line'] = () => {
							if(s.transcriptTextArray.length > 0){
								const x = t.refs.currentPara.listened.innerText.trim()
								if( x.slice(-1) !== '.' ) t.refs.currentPara.listened.innerText += '.'
							...
							t.fun.addPara()
							t.refs.outputCon.scrollTo(0, t.refs.outputCon.scrollHeight)
						}
						s.cmds['full stop'] = () => {
							if(s.transcriptTextArray.length > 0){
								t.refs.currentPara.listened.innerText = t.refs.currentPara.listened.innerText.trim() + '.'
								t.refs.currentPara.listening.innerHTML = null
							}
							else{
								t.refs.currentPara.listened.innerText = null
						...
						s.cmds['copy text'] = () => {
							...
							let text = ''
							const p = t.refs.outputCon.querySelectorAll('span.listened')
	...
	t.guide = (() => {
		const guide = {
			isVisible: window.localStorage.getItem('onLoadShowGuide') === 'false' ? false : true
		}
		guide.show = () => {
